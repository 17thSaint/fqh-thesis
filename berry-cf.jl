...
function get_expval(vers,n,p,hdf5_data,chosen_qpart,dtheta,samp_freq,log_form=false,acc_matrix = Matrix{Vector{Int}}(undef,(0,0)),all_pascal = Vector{Vector{Int}}(undef,0),all_derivs = Vector{Vector{Any}}(undef,0))
...
x_rads = [0.01*rm1 + j*(1.3*rm1)/10 for j in 0:9]
...
n_berry,p_berry = np_vals[which_np_getberry]
		...
		#
		radii_data_1q[3] = radii_data_1q[3][1:400]
		radii_data_1q[2] = radii_data_1q[2][:,1:400]
		...
		berry_calc_1q = get_expval(flux_type,n_berry,p_berry,radii_data_1q,1,-0.001,corr_length_1q,log_form)
...
println(berries_1q[5,2]/rads_1q[5,2]^2,", ",errors_1q[5,2]/rads_1q[5,2]^2)
=#
